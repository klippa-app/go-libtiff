...
import (
	...
	"github.com/klippa-app/go-libtiff/tiff2ps"
...
var _ = Describe("tiff2ps", func() {
	...
	It("shows the help text", func() {
		err := tiff2ps.Run(ctx, []string{"-help"})
		// For some reason tiff2ps works different from the other tools.
		// It outputs the help on stderr and returns exit code 1.
		Expect(err).To(MatchError(sys.NewExitError(1)))
		Expect(stdout.String()).To(BeEmpty())
		Expect(stderr.String()).To(ContainSubstring("usage: tiff2ps"))
	})
})
